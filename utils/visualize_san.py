...
def generate_visualizations(model, config, voc, src_str = '((()))(())()', run_name = 'SAN', iteration = 0,score = 1.0, device = 'cuda:1'):
	...
	def visualize_attn(src, src_str):
		...
		for i in range(config.depth):
			for j in range(config.heads):
				...
				plt.title('Attention Weights Layer: {} Head: {} (It: {}, Score: {})'.format(i+1, j+1, iteration, np.round(score, 3)), size = 20)
				fig = g.get_figure()
	...
	g.set_title('Embeddings (It: {}, Score: {})'.format(iteration, np.round(score)))
	fig = g.get_figure()
	...
	f.suptitle("Iteration: {}, Score: {}".format(iteration, np.round(score, 3)))
	...
	g.set_title('Values')
	fig = g.get_figure()
	fig.suptitle("Iteration: {}, Score: {}".format(iteration, np.round(score, 3)))
	...
	plt.plot(src_chars, dlratio,'--', marker = 's', markersize = 12, color = 'c', label = 'Depth : Length')
	plt.legend(loc="upper right")
	plt.title("Output of Self-Attention Block (It: {}, Score: {})".format(iteration, np.round(score, 3)))
	plt.grid()
	...
	g1.set_title('Decoder Weights (It: {}, Score: {})'.format(iteration, np.round(score, 3)))
